	...
	foreach ($tripDetails as $tripDetail) {
		foreach ($tripDetail as $day) {
			if (preg_grep($searchStringPattern, $day)) { //Ord: /\\b$search\\b/i Alla: /$search/i
				...
				echo '<div style="margin-left:auto;font-weight:bold;"><img style="height:0.9em;" src="img/house.php?color='.str_replace("#","",$darkColor).'"> <a target="_blank" style="color:'.$color.';" href="https://www.google.com/maps/?q='.str_replace(" ","",$day["AccommodationCoordinates"]).'">'.explode(",",$day["Accommodation"])[0]." ".$day["AccommodationCountry"].'</a></div></div>';
