...
<div class="bar tripBar">
    ...
    <div style="flex:1;text-align:center;">
        <select id="selectTripTypeAll" onchange="window.location.href = 'trip.php?id='+this.value" style="background-color:<?php echo $darkColor; ?>;color:#fff;padding:2pt;display:inline-block;">
...
<div class="bar tripBar" style="background-color:<?php echo $trip_settings[$tripBlocks[urldecode($_GET["id"])]["TripType"]]["Color"]; ?> !important">
    <div style="flex: 1;">
        <div class="bar-trip-button" style="text-align:left;">
    ...
    <div style="flex:1;text-align:center;">
        <select id="selectTripTypeCurrent" onchange="window.location.href = 'trip.php?id='+this.value" style="background-color:<?php echo $trip_settings[$tripBlocks[urldecode($_GET["id"])]["TripType"]]["Color"]; ?>;color:#fff;padding:2pt;display:inline-block;border-color:#fff;">
...
<!--<div class="tripBar">
    <div style="flex: 1;text-align:center;"><?php echo urldecode($_GET["id"]); ?> <span id="lang-trip-to"><?php echo $translation["trip"]["to"] ?? "to"; ?></span> <?php echo $tripBlocks[urldecode($_GET["id"])]["OverallDestination"]; ?></div>
</div>-->

<!-- Andra resor navigering slut -->

<div style="width:800pt;margin:auto;max-width:100%;">
...
<div class="lightColor roundBorder boxShadow" style="padding:5pt;">
		<div class="tripDivision roundBorderTop" id="lang-trip-summary"><?php echo $translation["trip"]["summary"] ?? "Summary"; ?></div>
		<div class="roundBorderBottom">
            ...
            foreach ($parts as $part) {
                if ( substr($part,0,1) == "{" ) {
                    ...
                    echo '<div style="display:inline-flex;max-width:190pt;margin-top:1pt;margin-bottom:1pt;padding:2pt 2pt 0pt 2pt;border-radius:2pt;" class="node"><div style="'.$underline.'padding:1pt;" title="'.$title.'">'.$node.'</div>';
                ...
                if ( (sizeof($parts)) != $i ) {
                    echo '<img src="img/plans_arrow.php?color='.str_replace("#","",$darkColor).'" style="height:10pt;margin-top:-2.5pt;margin-left:3pt;margin-right:3pt;vertical-align:middle;transform: rotate(180deg);">';
                }
            ...
            <div style="padding-top:10pt;"><b id="lang-trip-overall-route"><?php echo ($translation["trip"]["overall-route"] ?? "Overall Route"); ?></b><b>:</b><br><span id="route"><?php
                ...
                if ( ($settings["sheet-sort-order"]["map"]["reversed"] ?? false) === true ) {
                    $places[1] = array_reverse($places[1]);
                }
                ...
                foreach ($places[1] as $place) {
                    //$marker_color
                    echo '<div style="margin-top:5pt;height:20pt;display:inline-block;white-space:nowrap;"><img src="img/plans_marker.php?color='.str_replace('#','',$darkColor).''.'&letter='.$nb[$i].'" style="height:18pt;vertical-align:middle;margin-top:-2.5pt;" /> '.explode(",",str_replace("@","",$place))[0].'</div> ';
                    $i++;
                }
            ...
            <tr><td colspan="2" class="tripDivision" style="color:#ffffff;text-align:center;" id="lang-trip-day-per-day"><?php echo ($translation["trip"]["day-per-day"] ?? "Day per day"); ?></td></tr>
            ...
            foreach ($tripDetails[$_GET["id"]] as $tripDetail) {
                ...
                if ( ( $settings["immich-settings"]["use-immich"] ?? null ) === true ) {
                    if (isset($settings["inactivate-day-photos-button"])) {
                        if ( !in_array($_GET["id"], explode(", ",$settings["inactivate-day-photos-button"]) ) ) { echo '<div class="iib-action-button" style="margin-top:10pt;">'.($translation["trip"]["show-photos"] ?? "Show photos").'</div>'; }
                    } else {
                        echo '<div class="iib-action-button" id="lang-trip-show-photos" style="margin-top:10pt;">'.($translation["trip"]["show-photos"] ?? "Show photos").'</div>';
                    }
                }
                ...
                echo '<tr><td style="padding-right:20pt;width:100pt;" class="sepLine"></td><td class="sepLine"><img style="height:0.9em;" src="img/house.php?color='.str_replace("#","",$darkColor).'" /> <div id="nightNumber" style="position:absolute;display:inline-block;background-color:'.$darkColor.';border-radius:20pt;height:15pt;width:15pt;color:#fff;font-size:8pt;text-align:center;line-height:15pt;margin-top:1pt;">'.$night_no.'</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                ...
                echo $tripDetail["Accommodation"].' &nbsp;&nbsp;<img style="height:0.9em;" src="img/coordinate.php?color='.str_replace("#","",$darkColor).'" /> '.$tripDetail["AccommodationCountry"].', '.$tripDetail["AccommodationCoordinates"];
