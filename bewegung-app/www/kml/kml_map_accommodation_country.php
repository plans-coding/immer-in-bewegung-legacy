    ...
    foreach ($tripNumber as $tripDetail) {
        if ( $tripDetail["AccommodationCountry"] == $_GET["country"] ) {
            ...
            if (!in_array($tripDetail["AccommodationCoordinates"], ["-", "", "?"])) {
                ...
                $kml .= '    <description><![CDATA[<b>'.($translation["map-pin"]["accommodation"] ?? 'Accommodation').':</b> ' . $tripDetail["Accommodation"] . '<br /><b>'.($translation["map-pin"]["date"] ?? 'Date').':</b> ' . $tripDetail["Date"] . ' <b>'.($translation["map-pin"]["trip"] ?? 'Trip').':</b> <a href="trip.php?id='.$tripNumber2ID[$tripDetail["TripNumber"]].'#'.$tripDetail["Date"].'">' . $tripNumber2ID[$tripDetail["TripNumber"]] . '</a> <b>'.($translation["map-pin"]["coordinates"] ?? 'Coordinates').':</b> ' . $original . ']]></description>' . PHP_EOL;
                $kml .= '    <Point>' . PHP_EOL;
